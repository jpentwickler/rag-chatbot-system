...
class TestDataLoading(unittest.TestCase):
    ...
    def test_vector_store_initialization(self):
        ...
        with patch('vector_store.chromadb.PersistentClient') as mock_client, \
            ...
            self.assertEqual(mock_client_instance.get_or_create_collection.call_count, 2)  # Two collections
    ...
    def test_vector_store_search_with_filters(self):
        ...
        with patch('vector_store.chromadb.PersistentClient') as mock_client, \
            ...
            mock_client_instance = Mock()
            mock_client.return_value = mock_client_instance
            mock_client_instance.get_or_create_collection.side_effect = [mock_catalog, mock_collection]
            
            store = VectorStore("./test_db", "test-model", 5)
            
            # Test search with filters
            results = store.search("machine learning", course_name="ML", lesson_number=3)
    ...
    def test_vector_store_add_course_metadata(self):
        ...
        with patch('vector_store.chromadb.PersistentClient') as mock_client, \
            ...
            lessons = [
                ...
                Lesson(lesson_number=2, title="Advanced Topics", lesson_link="http://lesson2.com")
            ]
            course = Course(
                title="Test Course",
    ...
    def test_search_results_from_chroma(self):
        ...
        self.assertEqual(results.metadata, [{'title': 'Course 1'}, {'title': 'Course 2'}])
...
"""
        ...
        try:
            ...
            with patch.object(processor, 'process_course_document') as mock_process:
                ...
                lessons = [
                    Lesson(lesson_number=1, title="Introduction to ML", lesson_link="https://ml-course.com/lesson1"),
                    Lesson(lesson_number=2, title="Supervised Learning", lesson_link="https://ml-course.com/lesson2")
                ]
                course = Course(
                ...
                chunks = [
                    CourseChunk(content="Intro content", course_title="Test ML Course", lesson_number=1, chunk_index=0),
                    CourseChunk(content="Supervised learning content", course_title="Test ML Course", lesson_number=2, chunk_index=1)
                ]
                mock_process.return_value = (course, chunks)
                
                # Test processing
                result_course, result_chunks = processor.process_course_document(temp_path)
